...
where
  coalesce(trim(public.site_content.payload #>> '{profile,name}'), '') = ''
  and coalesce(jsonb_array_length(public.site_content.payload -> 'techStacks'), 0) = 0
  and coalesce(jsonb_array_length(public.site_content.payload -> 'experience'), 0) = 0;

insert into public.projects (id, payload)
values
  (
    'prolific-safety',
    $${
...
insert into public.testimonials (id, payload)
values
  (
    't1',
    $${
